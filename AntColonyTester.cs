...
namespace AntColonyTSP
{
    public class AntColonyTester
    {
        ...
        private readonly int[] _threadCounts = { 2, 4, 6, 8, 12 };
        ...
        public void RunTests()
        {
            ...
            foreach (var size in _citySizes)
            {
                ...
                _configs.goalDistance = Convert.ToInt32(0.1 * _configs.cityCount * (_configs.minDistance + _configs.maxDistance));
                _configs.goalCost = Convert.ToInt32(0.1 * _configs.cityCount * (_configs.minCost + _configs.maxCost));
                
                var adjacencyMatrix = Operators.GetRandomAdjacencyMatrix(_configs.cityCount, _configs.minDistance, _configs.maxDistance, _configs.minCost, _configs.maxCost, _configs.noPathProbability);
        ...
        private double RunBenchmark(int citySize, int threads, Path?[,] matrix)
        {
            ...
            for (int i = 0; i < RunsPerTest; i++)
            {
                ...
                if (threads == 1)
                {
                    var colony = new AntColonySequential(matrix, _configs);
                    colony.Solve();
                }
                else
                {
                    var colony = new AntColonyParallel(matrix, _configs);
                    colony.Solve();
                }
        ...
        private void PrintTable51(Dictionary<int, Dictionary<int, double>> results)
        {
            ...
            string header = string.Format("| {0,-15} | {1,-15} | {2,-15} | {3,-15} | {4,-15} | {5,-15} | {6,-15} |", 
                "City count", "Sequential", "Parallel (2)", "Parallel (4)", "Parallel (6)", "Parallel (8)", "Parallel (12)");
            ...
            foreach (var size in _citySizes)
            {
                Console.WriteLine(string.Format("| {0,-15} | {1,-15:F2} | {2,-15:F2} | {3,-15:F2} | {4,-15:F2} | {5,-15:F2} | {6,-15:F2} |",
                    size, results[size][1], results[size][2], results[size][4], results[size][6], results[size][8], results[size][12])); 
            }
            Console.WriteLine(new string('-', header.Length));
        }
        ...
        private void PrintTable52(Dictionary<int, Dictionary<int, double>> results)
        {
            ...
            foreach (var size in _citySizes)
            {
                ...
                Console.WriteLine(string.Format("| {0,-15} | {1,-15:F1} | {2,-15:F2} | {3,-15:F2} | {4,-15:F2} | {5,-15:F2} | {6,-15:F2} |",
                    size, 1.0, seq / results[size][2], seq / results[size][4], seq / results[size][6], seq / results[size][8], seq / results[size][12]));
            }
            Console.WriteLine(new string('-', header.Length));
        }
    }
